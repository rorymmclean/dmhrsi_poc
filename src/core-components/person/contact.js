   ...
   const [state, setState] = React.useState({
      columns: [
       { title: 'CONTACT TYPE', field: 'CONTACT_TYPE' },
          {
         title: 'TYPE', field: '', render: rowData =>
            {
              let s = "";
       ...
       {
         title: 'ADDRESS', field: '', render: rowData =>
         {
           const addr = [ rowData?.ADDRESS_1, rowData?.ADDRESS_2, rowData?.ADDRESS_3, rowData?.COUNTRY, rowData?.STATE, rowData?.CITY, rowData?.POSTAL_CODE ].filter( item => item )
           
          return <Typography type={ 'h3' }>{ `${ addr.join(" ,")}` }</Typography>
         }
       },
       {
         title: 'Primary', field: '', render: rowData =>
         {
           
           return <Checkbox defaultChecked disabled
             checked={ rowData?.PRIMARY_FLAG === "Y" }
           />
         }
       },
{
      ...
      editable: 'never',
      title: 'Edit',
  render: rowData => <EditContact  ENTITITY_TYPE={"PERSON"}  rowData={ rowData } onSave={(result) => {
        setState(prevState => {
  ...
  return (
    <div className="m-sm-30">
      <JPGrid container direction={'row'} justify={'flex-end'}>
        ...
        <AddContact ENTITITY_TYPE={"PERSON"} onSave={(result) => {
        setState(prevState => {
          const data = [...prevState.data];
          data.unshift(result);
