                ...
                <JPGrid container direction={'row'} justify={'flex-end'}>
                  ...
                  <JPGrid item marginRight={3} marginLeft={3}>
                    <Button
                      ...
                      variant={ 'outlined' }
                      disabled={ !value?.PROJECT_ID?.length || !data?.TASK_NAME?.length || !data?.SERVICE_TYPE?.length || !data?.TASK_NBR?.length }
                        color={!value?.PROJECT_ID?.length || !data?.TASK_NAME?.length || !data?.SERVICE_TYPE?.length || !data?.TASK_NBR?.length ? null : 'info' }
